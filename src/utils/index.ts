...
export function getTimeDifference(timestamp: any | number | Date) {
  const currentTime = Date.now();
  const difference = currentTime - timestamp;

  // 计算秒、分、小时、天、周、月和年的时间差
...
export function translateDataToTree(data: any[]) {
  const parent = data.filter((value: { parentId: string | null }) => value.parentId === "undefined" || value.parentId === null);
  const children = data.filter((value: { parentId: string | null }) => value.parentId !== "undefined" && value.parentId !== null);
