 ...
 * @returns {*}
 */
export function convertRouter(asyncRoutes: McRouteRecord[]) {
    ...
    if (route.component) {
      ...
      if (component)
        component[0] === "Layout"
          ...
          : (route.component = () => import(/* @vite-ignore */ `@/${component[0].replace(/@\/*/, "")}.vue`));
 ...
 * @returns {[]}
 */
export function filterRoutes(routes: McRouteRecord[], rolesControl: boolean, baseUrl = "/"): McRouteRecord[] {
  return routes
    .filter((route: McRouteRecord) => (rolesControl && route.meta.guard ? hasPermission(route.meta.guard) : true))
    .flatMap((route: McRouteRecord) =>
      baseUrl !== "/" && route.children && route.meta.levelHidden ? [...route.children] : route
    )
    .map((route: McRouteRecord) => {
      route = { ...route };
 ...
 * @returns {*} matched
 */
export function handleMatched(routes: McRouteRecord[], path: string): McRouteRecord[] {
  return routes
    .filter((route: McRouteRecord) => (route?.childrenPathList || []).indexOf(path) + 1)
    .flatMap((route: McRouteRecord) => (route.children ? [route, ...handleMatched(route.children, path)] : [route]));
}
 ...
 * @param tag route页信息
 */
export function handleTabs(tag: McRoute | McRouteRecord): any {
  let parentIcon = null;
  if ("matched" in tag)
  ...
  const path = handleActivePath(<McRoute>tag, true);
 ...
 * @returns {string|*}
 */
export function handleActivePath(route: McRoute, isTab = false) {
 ...
 * @returns {*} Name数组
 */
export function getNames(routes: McRouteRecord[]): string[] {
  return routes.flatMap((route: McRouteRecord) => {
    const names = [];
