...
import { gp } from '@gp'
import { McRouteRecord } from '/#/router'
...
export const useRoutesStore = defineStore('routes', {
  ...
  actions: {
    ...
    async setRoutes(mode = 'none') {
      ...
      //   } = await getList()
      //   if (!isArray(list))
      //     gp.$baseMessage('路由格式返回有误！', 'error', 'mc-hey-message-error')
      ...
      const accessRoutes = filterRoutes([...constantRoutes], true)
    ...
    changeMenuMeta(options: OptionType) {
      function handleRoutes(routes: McRouteRecord[]) {
