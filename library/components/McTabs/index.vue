...
const initNoCLosableTabs = (routes: McRouteRecord[]) => {
 ...
 */
const addTabs = async (tag: McRoute | McRouteRecord) => {
...
<template>
  <div class="mc-tabs">
    <mc-fold v-if="layout === 'common'" />
    <el-tabs v-model="tabActive" class="mc-tabs-content" :class="{
      ['mc-tabs-content-' + theme.tabsBarStyle]: true,
    }" type="card" @tab-click="handleTabClick" @tab-remove="handleTabRemove">
        ...
        <template #label>
          <span style="display: inline-block" @contextmenu.prevent="openMenu">
            <template v-if="theme.showTabsIcon">
              <mc-icon v-if="item.meta && item.meta.icon" :icon="item.meta.icon" :is-custom-svg="item.meta.isCustomSvg" />
              <!--  如果没有图标那么取第二级的图标 -->
              <mc-icon v-else :icon="item.parentIcon" />
            </template>
    ...
    <el-dropdown placement="bottom-end" popper-class="mc-tabs-more-dropdown" @command="handleCommand"
      @visible-change="handleVisibleChange">
      <span class="mc-tabs-more" :class="{ 'mc-tabs-more-active': active }">
        <span class="mc-tabs-more-icon">
          <i class="box box-t"></i>
          <i class="box box-b"></i>
        </span>
      </span>
      <template #dropdown>
        <el-dropdown-menu class="tabs-more">
          <el-dropdown-item command="refreshThisTab">
            <mc-icon icon="refresh-line" />
            <span>
              {{ translateTitle('刷新') }}
            </span>
          </el-dropdown-item>
          <el-dropdown-item command="closeOthersTabs">
            <mc-icon icon="close-line" />
            <span>
              {{ translateTitle('关闭其他') }}
            </span>
          </el-dropdown-item>
          <el-dropdown-item command="closeLeftTabs">
            <mc-icon icon="arrow-left-line" />
            <span>
              {{ translateTitle('关闭左侧') }}
            </span>
          </el-dropdown-item>
          <el-dropdown-item command="closeRightTabs">
            <mc-icon icon="arrow-right-line" />
            <span>
              {{ translateTitle('关闭右侧') }}
            </span>
          </el-dropdown-item>
    ...
    <ul v-if="visible" class="contextmenu el-dropdown-menu" :style="{ left: left + 'px', top: top + 'px' }">
      ...
      <li class="el-dropdown-menu__item" :class="{ 'is-disabled': !visitedRoutes.indexOf(hoverRoute) }"
        @click="closeLeftTabs">
        <mc-icon icon="arrow-left-line" />
