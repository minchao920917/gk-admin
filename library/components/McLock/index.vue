<script lang="ts" setup>
  ...
  const background = ref(
    `https://picsum.photos/1920/1080?random=${Math.random() * 200}`
  )
  const randomBackground = () => {
    background.value = `https://picsum.photos/1920/1080?random=${
      Math.random() * 200
    }`
  }
    ...
    formRef.value.validate(async (valid: boolean) => {
      if (valid) {
        ...
        setTimeout(async () => {
          ...
          const el = document.querySelector('.mc-side-bar') as HTMLElement
...
<template>
  <mc-icon v-if="theme.showLock" icon="lock-line" @click="handleLock" />
  <transition v-if="theme.showLock" mode="out-in" name="fade-transform">
    <div v-if="lock" class="mc-screen-lock">
      <div
        class="mc-screen-lock-background"
        :style="{
      ...
      <div class="mc-screen-lock-content">
        <div class="mc-screen-lock-content-title">
          <el-avatar :size="180" :src="avatar" />
          <mc-icon :icon="lockIcon ? 'lock-line' : 'lock-unlock-line'" />
          {{ title }} {{ translateTitle('屏幕已锁定') }}
        </div>
        <div class="mc-screen-lock-content-form">
