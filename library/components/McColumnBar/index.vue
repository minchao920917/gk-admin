...
<template>
  <el-scrollbar class="mc-column-bar-container" :class="{
    'is-collapse': collapse,
    ['mc-column-bar-container-' + theme.columnStyle]: true,
  }">
    <mc-logo />
    <el-tabs v-model="tab.data" tab-position="left" @tab-click="handleTabClick">
      <template v-for="(item, index) in routes" :key="index + item.name">
        <el-tab-pane :name="item.name">
          <template #label>
            <div class="mc-column-grid" :class="{
              ['mc-column-grid-' + theme.columnStyle]: true,
            }" :title="translateTitle(item.meta.title)">
              <div>
                <mc-icon v-if="item.meta.icon" :icon="item.meta.icon" :is-custom-svg="item.meta.isCustomSvg" />
                <span>
    ...
    <el-menu :background-color="menuVariables['column-second-menu-background']" :default-active="activeMenu.data"
      :default-openeds="defaultOpeneds" mode="vertical" :unique-opened="uniqueOpened">
      <el-divider>
        {{ translateTitle(tabMenu ? tabMenu.meta.title : '') }}
      </el-divider>
      <template v-for="item in partialRoutes" :key="item.path">
        <mc-menu v-if="!item.meta.hidden" :item="item" />
      </template>
    </el-menu>
  </el-scrollbar>
</template>
