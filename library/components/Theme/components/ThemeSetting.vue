...
<template>
  ...
  <ul v-if="theme.showThemeSetting" class="mc-theme-setting">
    <li @click="handleOpenTheme">
      <a>
        <mc-icon icon="brush-2-line" />
        <p>{{ translateTitle('主题配置') }}</p>
      </a>
    </li>
    <li @click="getCode">
      <a>
        <mc-icon icon="links-line" />
        <p>{{ translateTitle('个人博客') }}</p>
      </a>
    </li>
    <li @click="getDonate">
      <a>
        <mc-icon icon="money-cny-box-line" />
        <p>{{ translateTitle('打赏作者') }}</p>
      </a>
    </li>
    <li @click="removeLocalStorage">
      <a>
        <mc-icon icon="delete-bin-4-line" />
        <p>
          {{ translateTitle('清理缓存') }}
        </p>
      </a>
